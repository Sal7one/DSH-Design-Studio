...
function apply(ctx) {
  ...
  // /bin/sh -c with bounded collected output, an abort-driven timeout, and a
  ...
  async function listSystems(filterSessionId) {
    ...
    for (const e of entries) {
      ...
      const sysSessionId = typeof sysMeta.sessionId === "string" && sysMeta.sessionId ? sysMeta.sessionId : null;
      if (filterSessionId !== undefined && filterSessionId !== null && sysSessionId !== null && sysSessionId !== String(filterSessionId)) continue;
      ...
      out.push({ slug: e.name, sessionId: sysSessionId, orphan, createdAt: sysMeta.createdAt || null, count: files.length, hasPrompts, hasPreview, hasTokenCss, files });
    }
  ...
  async function createSystem(slug, sessionId) {
    ...
    // folders are created on demand, so empty dirs never appear in output.
  ...
  async function deletePreset(id) {
    ...
    const res = await sh({ command: "rm -f " + JSON.stringify(PRESETS_DIR + "/" + id + ".json") + " " + JSON.stringify(PRESETS_DIR + "/" + id + ".token.css"), workdir: await rootPath() });
  ...
  const tool = defineTool({
    name: "design_studio",
    description:
      "Manage the Design Studio under temp_design_folder/: list/create/read/write/zip design systems (html+css+js mockups), reveal the zip in Finder, CRUD identity presets (apply writes css/token.css + copies logo assets), chat with the design agent (a real harness subagent that lists, reads and edits the design files itself; persisted per-system history), persist the OpenRouter vision-model config, and run a vision review (image + brief -> honest GOOD|POOR verdict from the operator-chosen OpenRouter model). Writes are scoped to temp_design_folder/ design-system folders only. The OpenRouter key is a credential reference (OPENROUTER_API_KEY) and its value is never returned or rendered. DESIGN SYSTEMS ARE UI MOCKUPS ONLY: html + css + minimal presentation js — never full app or game logic (a tic-tac-toe design shows the board look, not playable rules); every create scaffolds a color palette + typography in css/token.css and design_prompts_forcoders.md.",
    parameters: {
      action: { type: "string", required: true, enum: ["list", "all", "create", "read", "write", "zip", "reveal", "presets", "preset_save", "preset_apply", "preset_delete", "review", "agent", "agent_history", "sweep", "delete", "config"], description: "Which studio operation to run." },
    ...
    async execute(args, exec) {
      ...
      switch (args.action) {
        ...
        case "all": {
          const systems = await listSystems(null);
          return JSON.stringify(systems.map((s) => ({ slug: s.slug, sessionId: s.sessionId, orphan: s.orphan, createdAt: s.createdAt, files: s.count, hasPreview: s.hasPreview })), null, 2);
        }
        case "create": {
