      ...
      async function askDeepSeek() {
        ...
        try {
          ...
          try {
            ...
            const scoped = sessionsSvc && typeof sessionsSvc.scope === 'function' && sessionId ? sessionsSvc.scope(sessionId) : undefined
            conv = scoped ? scoped.conversation : undefined
          } catch (_) {}
          if (!conv || typeof conv.send !== 'function') {
            setErr('main chat not reachable from this tab — use "Save only" and ask DeepSeek in the main conversation instead')
            return
          }
          await conv.send(promptText)
          setLog('sent to DeepSeek in the main chat — it edits the files itself and invokes the design agent when it needs vision')
          setSelection(null)
          setEditMsg('')
          setDsReply('')
        } catch (e) {
                  ...
                  selection
                    ? h(
                        ...
                        h(
                          ...
                          h('span', { className: 'ds-hint' }, 'Ask DeepSeek sends the request to your main chat — DeepSeek edits the files itself and calls the design agent when it needs vision. "Make the design agent do it" keeps the agent-chat flow. Both save ' + sel.slug + '/EDIT_REQUEST.md. One element at a time; pick again with 🎯 Select.'),
                        ),
                      )
                    : null,
                ),
