// Design Studio — CLIENT half (dynamic cordis plugin, plain JS, no JSX)
// Slots used (queried): conversation.view (id design-studio, LAST tab), settings.section (ids all-designs + design-studio),
// tool.view.cordis (key self). All data flows through host.call (read-only projection style).
// RPC hygiene: args are plain JSON only — undefined keys are omitted before every host.call.
// UI: high-density command-center look (self-contained CSS; no CDNs), dark navy + periwinkle primary.
return {
  inject: ['timer'],
  apply(ctx) {
    ...
    function RunPanel() {
      ...
      React.useEffect(() => {
        ;(async () => {
          try {
            const sys = await call('studio.list', {})
            const cfg = await call('studio.config')
            setSt({
              systems: sys.ok ? sys.data.length : -1,
              keyConfigured: Boolean(cfg.ok && cfg.data && cfg.data.key && cfg.data.key.configured),
              keySource: cfg.ok && cfg.data && cfg.data.key ? cfg.data.key.source : null,
              harnessRoute: Boolean(cfg.ok && cfg.data && cfg.data.harness && cfg.data.harness.routeRegistered),
            })
          } catch (e) {
            setSt({ error: String((e && e.message) || e) })
          }
        })()
      }, [])
      ...
      return h(
        'div',
        { className: 'ds-runpanel' },
        h('div', { className: 'ds-runpanel-title' }, 'Design Studio — running'),
        st === null
          ? h('div', { className: 'ds-hint' }, 'checking host…')
          : st.error
            ? h('div', { className: 'ds-err' }, st.error)
            : h(
                'div',
                { className: 'ds-runpanel-body' },
                h('div', null, st.systems >= 0 ? st.systems + ' design system(s) under temp_design_folder/' : 'studio host unreachable'),
                h(
                  'div',
                  null,
                  'OpenRouter key: ' + (st.keyConfigured ? 'configured' + (st.keySource ? ' (source ' + st.keySource + ')' : '') : 'not configured — set it in Settings → Design Studio'),
                ),
                h('div', null, 'Vision transport: ' + (st.harnessRoute ? 'harness openrouter route (ctx.llm + attachments)' : 'plugin curl client (no openrouter route registered yet)')),
                h('div', null, 'Live preview served at /design-studio/<slug>/html/index.html (iframe in the Design Studio tab).'),
                h('div', { className: 'ds-hint' }, 'Open the Design Studio tab in this conversation for preview / zip / vision review / design agent / file drops.'),
              ),
      )
    }
    ...
    slots.inject('tool.view.cordis', () => slots.register({ name: 'tool.view.cordis', key: 'self' }, () => h(RunPanel)))
  },
}
